...
Result MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // account for the latency time 100 ms = 2*dt -> 3rd value
  for (int i = delta_start; i < delta_start + 2; i++) {
    vars_lowerbound[i] = delta_old;
    vars_upperbound[i] = delta_old;
  }
  ...
  for (int i = a_start; i < a_start+2; i++) {
    vars_lowerbound[i] = a_old;
    vars_upperbound[i] = a_old;
  }
